...
import com.onea.sidot.gestioneau.config.KafkaProperties;
...
class GestioneauKafkaResourceIT {
    ...
    void setup() {
        ...
        GestioneauKafkaResource kafkaResource = new GestioneauKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/gestioneau-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/gestioneau-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
